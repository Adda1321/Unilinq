...
class index extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      communityName: '',
      participant_number:'',
      unlimited_participant:true,
      categoryId: props.route.params.category_data,
      monthId:'',
  ...
  communityAdd = async () => {
    ...
    console.log("Rrrrrrrrokok======>",this.state.participant_number,checkParticipants)
    if (
      ...
      if (this.state.capturedImage != '') {
        ...
        await task.snapshot.ref.getDownloadURL().then(downloadURL => {
          ...
          // this.props.navigation.navigate('eventComplete', {check: false});
        ...
        addEvent(
          ...
          "https://schoolreadinglist.co.uk/wp-content/uploads/2012/05/literaryeventsandfestivals.jpg",
          pend,
          this.state.timestart,
            this.state.timeend,
            this.state.datestart,
      ...
      <View style={{backgroundColor: '#fff', flex: 1}}>
        <ScrollView  keyboardShouldPersistTaps={'handled'}>
          ...
          <View style={{paddingHorizontal: 20}}>
            <Text style={styles.createCommunityTextHeader}>New Event</Text>
            <View
              style={{
                flexDirection: 'row',
   ...
   { this.state.categoryCheck.isValidUser && (
    ...
    <Text style={{color:'#E4434E',fontFamily:'Poppins-Bold',fontSize:12}}>{this.state.categoryCheck.message}</Text>
            ...
            <View style={{marginTop: 30}}>
              <TimeLoc  timecheck ={(val,res,st)=>this.compareTime(val,res,st)} time={this.state.timecheck} timend={this.state.timecheckend} datecheck ={(val,res,st)=>this.compareDate(val,res,st)} category={(data)=>this.monthList(data)} dropdownCheck={(id,val)=>this.dropdownCheck(id,val)} unlimitedCheck={(val)=>this.unlimitedCheck(val)} val = {this.state.value}  changetext={(val)=>this.changeText(val)} communityType={this.state.communityType} timerequired={this.state.timerequired} addresscheck={this.state.addressCheck} participantsRequired={this.state.participantsrequired}/>
            </View>
            {/* ///////////////////////////////////////// TIME LOCATION END//////////////////////////// */}
