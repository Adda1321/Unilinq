  ...
  const acceptMember =(id,community_id,index)=>{
    ...
    return (
        <View style={{flex:1}}>
          ...
          {props.route.params.check ==true && communityMemberss.length >0 &&(
            <>
           {communityMemberss.map((item,index)=>(
             <>
              <View style={{paddingHorizontal:23,paddingTop:18}}>
              <Text style={{fontFamily:'Poppins-Bold',fontSize:14,color:'#b7b7b7',letterSpacing:-0.5}}>Pending</Text>
            </View>
 <View style={styles.pendingcommunityMemberAdminInfoContainer}>
 <View style={{flexDirection:'row',alignItems:'center',marginTop:15}}>
   <View style={[styles.communityDetailsInfoInnerContainer,{width:'80%'}]}>
     <TouchableOpacity onPress={()=>navigation.navigate('UserScreen')}>
          <ImageView src={{uri:item?.image}} imageStyle={styles.communityDetailsCircularImage} />
          </TouchableOpacity>
          <View style={{marginLeft:10}}>
              <Text style={styles.communityDetailsAdminInfoTextOne}>{item?.first_name} {item?.last_name}</Text>
              <Text style={[styles.communityMembersAdminInfoTextTwo,{marginTop:2}]}>Pending</Text>
          </View>
          </View>
...
<TouchableOpacity onPress={()=>deleteMember(item.id,props.route.params.data.id,index)}>
<ImageView src={require('../../images/crossImage.png')} imageStyle={{width:19,height:19,marginRight:20}} />
</TouchableOpacity>
<TouchableOpacity onPress={()=>acceptMember(item.id,props.route.params.data.id,index)}>
<ImageView src={require('../../images/tickimage.png')} imageStyle={{width:24,height:18,marginLeft:20}} />
</TouchableOpacity>
</View>
</View>
      </View>
...
/>
        ...
        <View style={styles.communityDetailsInfoInnerContainer}>
          ...
          {props.route.params.community.length == 1 ? 
